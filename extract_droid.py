#!/usr/bin/env python3
"""
Extract Droid CLI / Factory session data.

Includes user and assistant messages, tool use and results, thinking blocks,
todo snapshots, compaction summaries, and optional per-session settings
metadata from ~/.factory/sessions.
"""
...
def extract_droid_sessions():
    sessions_dir = find_droid_sessions_dir()
    if not sessions_dir:
        return []
                ...
                for line in handle:
                    ...
                    if entry_type == "session_start":
                        ...
                        if (
                            ...
                            or normalized.get("thinking")
                            or normalized.get("tool_uses")
                            or normalized.get("tool_results")
                        ):
                        ...
                        ended_at = obj.get("timestamp", ended_at)
...
def main():
    ...
    with_tools = sum(
        ...
        if any("tool_uses" in message or "tool_results" in message for message in conv["messages"])
    )
    with_thinking = sum(1 for conv in conversations if any("thinking" in message for message in conv["messages"]))
    with_settings = sum(1 for conv in conversations if "settings" in conv)
